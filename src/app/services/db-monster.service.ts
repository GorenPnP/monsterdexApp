...
export class DbMonsterService {
            ...
            if (ready) {
              ...
              this.allMonsters.asObservable().subscribe(mon => {
                ...
                if (this.monsters.getValue().length !== this.lastMonster) {
                  const mons = mon.slice(0, this.lastMonster);
   ...
   */
  private updateMonsters(mons: Monster[], fromFilter: boolean = false): void {

    const allMons: Monster[] = this.allMonsters.getValue();
    for (const mon of mons) {
      allMons[mon.id - 1] = mon;
    }

    // filter is activated, ignore allMonsters
    if (fromFilter) { this.monsters.next(mons); return; }
  ...
  private async getMonstersByIds(neededIds: number[]): Promise<Monster[]> {
    ...
    const prepopulatedMons: Monster[] = [];

    const allMon = this.allMonsters.getValue().slice();
    let tempMon;
    for (const neededId of neededIds) {
      // dont go over allMonster's boundaries
      if (neededId > this.NUM_MONSTER) { continue; }

      tempMon = allMon[neededId - 1];
    ...
    return this.db.executeSql(query, values).then(async data => {
      return this.dataToMonster(data).then(mons => {
        ...
        if (mons.length !== values.length) {
          this.messageService.alert('Nicht alle Monster gefunden',
                                    'in GET MONSTER: could not get all monster with ids: ', values,
                                    ' found: ', this.databaseService.listIds(mons));
          return [];
        }

        // sort concatenated lists in place
        const returnList = prepopulatedMons.concat(mons).sort((a, b) => a.id < b.id ? -1 : 1);
        return returnList;
      });
    });
  }
  ...
  async findByWord(nameValue: string, callCombineAfterwards: boolean = true): Promise<void> {
    ...
    const query = `SELECT * FROM monster_monster WHERE name LIKE ? OR id=?`;
  ...
  async findByType(typeIdList: number[], operandIsOr: boolean, callCombineAfterwards: boolean = true): Promise<void>  {
    ...
    // construct query
    let query: string = 'SELECT * FROM monster_monster WHERE id IN (SELECT a.monster_id FROM ' +
                        '(SELECT monster_id, COUNT(*) AS num FROM monster_monster_typen WHERE typ_id IN (';
    const values = [];
  ...
  private async getAttacken(monId: number): Promise<number[]> {
    ...
    const query = `SELECT attacke_id FROM monster_monster_attacken WHERE monster_id=?`;
  ...
  private async dataToMonster(data): Promise<Monster[]> {
    ...
    for (let i = 0; i < data.rows.length; i++) {
      ...
      await Promise.all([
        ...
        this.dbTypen.getMonsterTypen(item.id),
  ...
  async getMonstersByAttacke(attId: number): Promise<Monster[]> {
    ...
    const query = `SELECT monster_id FROM monster_monster_attacken WHERE attacke_id=?`;
    ...
    return this.db.executeSql(query, [`${attId}`]).then(data => {
      ...
      return this.getMonstersByIds(ids).then(mons => mons);
  ...
  async getAehnlicheFormen(monsId: number): Promise<number[]> {
    return this.db.executeSql('SELECT mon_2 FROM monster_monster_verschFormen WHERE mon_1=?', [`${monsId}`]).then(data => {
      ...
      return returnList;
    }).catch(err => { this.messageService.error('Konnte keine ähnlichen Formen zu dem Monster laden.',
                                                'error loading aehnliche formen of', monsId, err);
                      return []; });
  }
  ...
  async getGegenteilmonster(monsId: number): Promise<number[]> {
    return this.db.executeSql('SELECT mon_2 FROM monster_gegenteilmonster WHERE mon_1=?', [`${monsId}`]).then(data => {
      ...
      return returnList;
    }).catch(err => { this.messageService.error('Konnte keine Gegenteilmonster zu dem aktuellen Monster laden',
                                                'error loading gegenteilmonster of', monsId, err);
                      return []; });
  }
  ...
  async getEvolution(monId: number): Promise<Monster[][]> {
    const anchestors = await this.getAnchestors(monId);
    const predecessor = await this.getPredecessors(monId);
  ...
  async getAnchestors(monId: number): Promise<Monster[]> {
    ...
    const query = 'SELECT vor FROM monster_evolution WHERE nach=?';
  ...
  async getPredecessors(monId: number): Promise<Monster[]> {
    ...
    const query = 'SELECT nach FROM monster_evolution WHERE vor=?';
  ...
  async getAllSortedByRang(sortingIndex: number): Promise<void> {
    ...
    // get sorting
    const query = 'SELECT * FROM monster_monster ORDER BY rang ' + (sortingIndex === 1 ? 'ASC' : 'DESC');
